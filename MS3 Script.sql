...
--make sure student has a GP
--make sure student can have a GP


GO
CREATE PROCEDURE HasGP 
@studentID int,
@semcode varchar(40),
@OUT INT OUTPUT
AS
DECLARE @EXIST INT;
SELECT @EXIST=COUNT(*) FROM Graduation_Plan WHERE semester_code=@semcode AND student_id=@studentID;
...
GO
CREATE PROCEDURE HasGPtoUpdate
@studentID int,
@OUT INT OUTPUT
AS
DECLARE @EXIST INT;
SELECT @EXIST=COUNT(*) FROM Graduation_Plan WHERE student_id=@studentID;
...
AS
DECLARE @EXIST INT;
SELECT @EXIST=COUNT(*) FROM Course WHERE name=@coursename;
...
SELECT @EXISTS=COUNT(*) FROM Student WHERE student_id=@studentID AND advisor_id=@advisorID;
IF @EXISTS =0
SET @OUT=0
ELSE 
BEGIN
...
DECLARE @HASGP INT;
SELECT @HASGP=COUNT(*) FROM Graduation_Plan 
WHERE semester_code=@semcode AND semester_credit_hours=@chr AND student_id=@studentID AND expected_grad_date=@EXP AND advisor_id=@advisorID

IF @HASGP =0
SET @OUT =1 --If no plan exists
ELSE
SET @OUT=2 --if a plan exists
...
DECLARE @OUT INT;
EXEC CanDeleteGPCourse 1,'W20',6,@OUT;
PRINT @OUT;

GO
CREATE PROCEDURE CanDeleteGPCourse
@studentID int,
@sem_code varchar(40),
@courseID int,
@OUT INT OUTPUT
...
select @EXISTS = COUNT(*) from Graduation_Plan
where Graduation_Plan.student_id = @studentID and Graduation_Plan.semester_code = @sem_code;

IF @EXISTS=0
SET @OUT = 0 -- NO GRADUATION PLAN
ELSE
BEGIN

select @gp_plan = Graduation_Plan.plan_id from Graduation_Plan
where Graduation_Plan.student_id = @studentID and Graduation_Plan.semester_code = @sem_code;



SELECT @EXISTS=COUNT(*) from GradPlan_Course
where GradPlan_Course.plan_id = @gp_plan and GradPlan_Course.course_id= @courseID and GradPlan_Course.semester_code = @sem_code

IF @EXISTS=0
SET @OUT=2 --NO COURSE TO DELETE
ELSE
SET @OUT=1 --PROCEED NO ERROR
...
DECLARE @EXIST INT;
SELECT @EXIST=COUNT(*) FROM Request WHERE request_id=@reqId AND advisor_id=@advisorID;
IF @EXIST=0
SET @OUT=3 --REQ DOESN'T EXIST
ELSE 
BEGIN


DECLARE @YOURS INT;
SELECT @YOURS=COUNT(*) FROM Request WHERE request_id=@reqId AND advisor_id=@advisorID;

IF @YOURS=0
SET @OUT=0 --NOT YOUR STUDENT
ELSE 
BEGIN


DECLARE @PENDING INT;
SELECT @PENDING=COUNT(*) FROM Request WHERE request_id=@reqId AND status='Pending';
IF @PENDING = 0
SET @OUT=2 --REQ NOT PENDING
ELSE 
SET @OUT=1 --PROCEED
...
GO
CREATE PROCEDURE getReqStatus
@reqId int,
@OUT INT OUTPUT
AS
DECLARE @STATUS VARCHAR(40);
SELECT @STATUS=status FROM Request WHERE request_id=@reqId;

IF @STATUS like '%accept%'
SET @OUT=1
ELSE 
SET @OUT=0
...
DECLARE @COMPARE VARCHAR(40);

SELECT @COMPARE =type FROM Request WHERE request_id=@reqId;
IF @COMPARE LIKE'%credit%' and @type LIKE '%credit%'
SET @OUT=1
ELSE 
BEGIN 
IF @COMPARE LIKE'%course%' and @type LIKE '%course%'
SET @OUT=1
ELSE 
SET @OUT=0
...
GO
CREATE PROCEDURE Procedures_Check_inSemester
...
GO
CREATE PROCEDURE ViewAdvStudents
@advisorID int
as
SELECT * FROM Student WHERE advisor_id=@advisorID;
GO
...
GO
Create Function StudentFinancialStatus(@student_id int)
returns bit
as
begin
declare @result bit = 1
DECLARE @installment_deadline date
DECLARE @installment_status varchar(40)

SELECT @installment_deadline = I.deadline , @installment_status = I.status
...
IF (current_timestamp > @installment_deadline AND @installment_status = 'notPaid')
set @result = 0
return @result
End;
...
as
 update Student
 set financial_status = dbo.StudentFinancialStatus(@student_id)
 where student_id = @student_id
 go

 GO
CREATE PROC Procedures_CheckStudent
    @student INT,
    @count INT OUTPUT
AS
BEGIN
